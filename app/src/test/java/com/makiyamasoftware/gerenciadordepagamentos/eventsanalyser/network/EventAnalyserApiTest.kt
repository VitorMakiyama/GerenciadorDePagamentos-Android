...
import com.makiyamasoftware.gerenciadordepagamentos.eventsanalyser.reports.ChartReport
...
class EventAnalyserApiTest {
    ...
    fun getRetrofitEventAnalyserService(url: HttpUrl): EventAnalyserApiService {
    ...
    fun getRetrofitEventsReportsService(url: HttpUrl): EventsReportsApiService {
    ...
    fun testPing() = runBlocking {
        ...
        val retrofit = getRetrofitEventAnalyserService(server.url("/"))
    ...
    fun test_getReportTypes() = runBlocking {
        ...
        val retrofit = getRetrofitEventsReportsService(server.url("/"))
        val result = retrofit.getReportTypes()

        val request = server.takeRequest()

        assertThat(request.path).isEqualTo("/reports/types")
        assertThat(result).isEqualTo(listOf("BASIC", "CHART"))
    }

    @Test
    fun test_getReportData_BASIC_REPORT() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"type": "BASIC", "details": {"weekly":"1,0","monthly":"4,0","yearly":"10","sigma":"0,5","start_date":"2026-05-16","end_date":"2026-06-08","total_occurrences":"10"}}"""))
    ...
    fun test_getReportData_CHART_REPORT() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"type": "CHART","details": { "data":[1,2,3,4],"x_labels": ["2026-05-16","2026-05-23","2026-05-30","2026-06-08"]}}"""))

        val retrofit = getRetrofitEventsReportsService(server.url("/"))
        val result = retrofit.getReportData(EventsReportType.CHART_DAILY.name, 1)

        val request = server.takeRequest()

        assertThat(request.path).isEqualTo("/reports?type=CHART&subject_id=1")
        assertThat(result).isEqualTo(
            EventsReportsData.ChartReportData(
                type = "CHART",
                details = ChartReport(data = listOf(1, 2, 3, 4), xLabels = listOf("2026-05-16", "2026-05-23", "2026-05-30", "2026-06-08",))
            )
        )
    }
}
